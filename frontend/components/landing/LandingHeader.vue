...
<template>
  <header
    ...
    :class="{ 'mm-landing-header--scrolled': headerScrolled, 'mm-landing-header--open': mobileNavOpen }"
  >
    <div class="mm-landing-header__bar mx-auto flex h-14 max-w-[1400px] items-center gap-3 px-4 sm:h-16 sm:px-8 lg:px-12">
      ...
      <div class="ml-auto flex items-center gap-2">
        ...
        <NuxtLink
          v-if="!authStore.isAuthenticated"
          to="/login"
          class="mm-landing-header__login-icon flex h-10 w-10 items-center justify-center rounded-full border border-[color:var(--mm-border)] text-[color:var(--mm-primary)] transition hover:bg-[color:var(--mm-primary-soft)] sm:hidden"
          aria-label="Войти"
        >
        ...
        <button
          v-if="authStore.isAuthenticated || isLanding"
          type="button"
