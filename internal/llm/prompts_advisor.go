...
Ответ — ТОЛЬКО один валидный JSON-объект, без markdown и без текста до/после:
{
...
Правила текста (критично):
- Пиши нормальными русскими словами: пробел только МЕЖДУ словами, никогда внутри слова.
- НЕЛЬЗЯ: «де по зит», «го до вых», «пред по чтительнее», «доход ы», «по это му».
- МОЖНО: «депозит», «годовых», «предпочтительнее», «доходы», «поэтому».
